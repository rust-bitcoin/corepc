...
fn raw_transactions__decode_script_segwit__modelled() {
    ...
    let decoded = match model {
        Ok(d) => d,
        Err(DecodeScriptError::Segwit(DecodeScriptSegwitError::Addresses(_))) if is_legacy_version => {
            println!("[SKIPPED] Segwit address validation not supported in this version");
    ...
    // Handle cases where segwit field might not be present
    if let Some(segwit) = &decoded.segwit {
        // Validate segwit-specific fields when available
        ...
        if is_legacy_version {
            println!("[NOTE] Segwit field not present in legacy version - skipping detailed validation");
            // Still do basic validation
            assert_eq!(decoded.hex, hex, "Input hex does not match output hex");
            assert!(!decoded.type_.is_empty(), "Script type should not be empty");
            return;
        }
        // Fallback validation for modern versions that should have segwit field
        if let Some(script_pubkey) = &decoded.script_pubkey {
            assert_eq!(script_pubkey, &script, "Script hex mismatch in script_pubkey field");
        } else {
            println!("[WARNING] Script hex not returned in decode_script response for segwit script");
        }
        // Check if address is present and valid
        if let Some(addr) = &decoded.address {
            let checked_addr = addr.clone().assume_checked();
            // For P2WPKH, we expect a witness program
            assert!(
                checked_addr.script_pubkey().is_witness_program(),
                "Invalid witness address: {:?}",
                checked_addr
        ...
        println!("[NOTE] Segwit field not present in decode_script response - using fallback validation");
    }
    // Additional validation that should work regardless of segwit field presence
    assert_eq!(decoded.hex, hex, "Input hex does not match output hex");
    assert!(!decoded.type_.is_empty(), "Script type should not be empty");
    // Type should indicate it's a witness script
    assert!(
