...
//! Auto-generated `into_model` conversions for Bitcoin Core `30` - rawtransactions.
//!
...
impl DecodePsbt {
    ...
    pub fn into_model(self) -> Result<model::DecodePsbt, DecodePsbtError> {
        ...
        Ok(model::DecodePsbt {
            // TODO(codegen): canonical field `psbt: Psbt` has no raw counterpart; needs manual reconstruction.
            psbt: todo!("no raw field for `psbt`"),
            fee: self.fee.map(Amount::from_btc).transpose().map_err(E::Fee)?,
        })
    }
}
...
impl DecodeRawTransaction {
    ...
    pub fn into_model(self) -> Result<model::DecodeRawTransaction, DecodeRawTransactionError> {
        ...
        // TODO(codegen): reconstruct `Transaction` from the decoded fields of `DecodeRawTransaction`.
        // The canonical `DecodeRawTransaction` is a newtype around `Transaction`, but the raw response is the
        // already-decoded value with no single field to convert. Needs manual assembly.
        Ok(model::DecodeRawTransaction(todo!("reconstruct Transaction from decoded fields")))
    }
}
