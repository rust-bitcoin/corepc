...
impl GetBlockStats {
    ...
    pub fn into_model(self) -> Result<model::GetBlockStats, GetBlockStatsError> {
        ...
        Ok(model::GetBlockStats {
            average_fee: Some(Amount::from_sat(self.average_fee)),
            average_fee_rate,
            average_tx_size: Some(crate::to_u32(self.average_tx_size, "average_tx_size")?),
            block_hash: Some(block_hash),
            fee_rate_percentiles: Some(fee_rate_percentiles),
            height: Some(crate::to_u32(self.height, "height")?),
            inputs: Some(crate::to_u32(self.inputs, "inputs")?),
            max_fee: Some(Amount::from_sat(self.max_fee)),
            max_fee_rate,
            max_tx_size: Some(crate::to_u32(self.max_tx_size, "max_tx_size")?),
            median_fee: Some(Amount::from_sat(self.median_fee)),
            median_time: Some(crate::to_u32(self.median_time, "median_time")?),
            median_tx_size: Some(crate::to_u32(self.median_tx_size, "median_tx_size")?),
            minimum_fee: Some(Amount::from_sat(self.minimum_fee)),
            minimum_fee_rate,
            minimum_tx_size: Some(crate::to_u32(self.minimum_tx_size, "minimum_tx_size")?),
            outputs: Some(crate::to_u32(self.outputs, "outputs")?),
            subsidy: Some(Amount::from_sat(self.subsidy)),
            segwit_total_size: Some(crate::to_u32(self.segwit_total_size, "segwit_total_size")?),
            segwit_total_weight,
            segwit_txs: Some(crate::to_u32(self.segwit_txs, "segwit_txs")?),
            time: Some(crate::to_u32(self.time, "time")?),
            total_out: Some(Amount::from_sat(self.total_out)),
            total_size: Some(crate::to_u32(self.total_size, "total_size")?),
            total_weight,
            total_fee: Some(Amount::from_sat(self.total_fee)),
            txs: Some(crate::to_u32(self.txs, "txs")?),
            utxo_increase: Some(self.utxo_increase),
            utxo_size_increase: Some(self.utxo_size_increase),
