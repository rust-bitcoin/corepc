...
/// Error when converting a `SignRawTransactionWithWalletError` type into the model type.
#[derive(Debug)]
pub enum SignRawTransactionWithWalletError {
    /// Conversion of the transaction `hex` field failed.
    Tx(encode::FromHexError),
    /// Conversion of one of the items in the `errors` field failed.
    Errors(SignErrorDataError),
}
...
impl fmt::Display for SignRawTransactionWithWalletError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            E::Tx(ref e) => write_err!(f, "conversion of the `hex` field failed"; e),
            E::Errors(ref e) => {
                write_err!(f, "conversion of one of the items in the `errors` field failed"; e)
            }
        }
    }
}
...
/// Error when converting a `SignErrorData` type into the model type.
#[derive(Debug)]
pub enum SignErrorDataError {
    /// Conversion of numeric type to expected type failed.
    Numeric(NumericError),
    /// Conversion of the `txid` field failed.
    Txid(hex::HexToArrayError),
...
impl fmt::Display for SignErrorDataError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl From<NumericError> for SignErrorDataError {
    fn from(e: NumericError) -> Self { Self::Numeric(e) }
}
