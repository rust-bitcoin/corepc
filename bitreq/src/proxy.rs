...
impl Proxy {
    ...
    /// Returns the filled buffer and its length. Max length is 4 + 1 + 255 + 2 = 262 bytes.
    fn socks5_connect_request(
        ...
        let port_off = 5 + host_bytes.len();
        buf[port_off] = (target_port >> 8) as u8;
        buf[port_off + 1] = (target_port & 0xff) as u8;
        Ok((buf, port_off + 2))
    }
