...
impl Proxy {
    ...
    fn socks5_connect_request(
        ...
        // A bracketed IPv6 literal such as `[::1]` does not parse as an
        // `IpAddr`, so strip one surrounding pair of brackets first.
        let ip_literal =
            target_host.strip_prefix('[').and_then(|h| h.strip_suffix(']')).unwrap_or(target_host);

        let body_end = match ip_literal.parse::<IpAddr>() {
            Ok(IpAddr::V4(v4)) => {
                buf[3] = 0x01;
...
mod tests {
    ...
    mod socks5_handshake {
        ...
        #[test]
        fn handshake_bracketed_ipv6_literal_uses_atyp_four() {
            // A bracketed IPv6 destination (e.g. `[::1]`) must have its
            // brackets stripped and be encoded with ATYP 0x04, not fall
            // through to the ATYP 0x03 domain path.
            let listener = TcpListener::bind("127.0.0.1:0").unwrap();
            let addr = listener.local_addr().unwrap();
            let proxy = Proxy::new_socks5(format!("127.0.0.1:{}", addr.port())).unwrap();
            ...
            let mut stream = std::net::TcpStream::connect(addr).unwrap();
            proxy.socks5_handshake_sync(&mut stream, "[::1]", 443).unwrap();
