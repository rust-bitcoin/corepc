...
impl Proxy {
    ...
    pub(crate) fn is_socks5(&self) -> bool {
        matches!(self.kind, ProxyKind::Socks5)
    }
    ...
    pub(crate) fn socks5_handshake_sync(
        ...
        stream.write_all(&greeting).map_err(Error::IoError)?;
        stream.flush().map_err(Error::IoError)?;

        let mut greeting_resp = [0u8; 2];
        stream.read_exact(&mut greeting_resp).map_err(Error::IoError)?;
        Self::socks5_check_greeting(&greeting_resp, expected_method)?;

        // 2. Username/password auth (RFC 1929), if credentials set
        if let Some(auth_req) = self.socks5_auth_request() {
            stream.write_all(&auth_req).map_err(Error::IoError)?;
            stream.flush().map_err(Error::IoError)?;

            let mut auth_resp = [0u8; 2];
            stream.read_exact(&mut auth_resp).map_err(Error::IoError)?;
            Self::socks5_check_auth(&auth_resp)?;
        }

        // 3. Connect request
        let req = Self::socks5_connect_request(target_host, target_port)?;
        stream.write_all(&req).map_err(Error::IoError)?;
        stream.flush().map_err(Error::IoError)?;

        // 4. Read response header
        let mut connect_resp = [0u8; 4];
        stream.read_exact(&mut connect_resp).map_err(Error::IoError)?;
        ...
        match connect_resp[3] {
            0x01 => { // IPv4: 4 bytes + 2 port
                let mut buf = [0u8; 6];
                stream.read_exact(&mut buf).map_err(Error::IoError)?;
            }
    ...
    pub(crate) async fn socks5_handshake_async(
        ...
        // 1. Greeting
        let (greeting, expected_method) = self.socks5_greeting();
        stream.write_all(&greeting).await.map_err(Error::IoError)?;
        stream.flush().await.map_err(Error::IoError)?;

        let mut greeting_resp = [0u8; 2];
        stream.read_exact(&mut greeting_resp).await.map_err(Error::IoError)?;
        Self::socks5_check_greeting(&greeting_resp, expected_method)?;

        // 2. Username/password auth (RFC 1929), if credentials set
        if let Some(auth_req) = self.socks5_auth_request() {
            stream.write_all(&auth_req).await.map_err(Error::IoError)?;
            stream.flush().await.map_err(Error::IoError)?;

            let mut auth_resp = [0u8; 2];
            stream.read_exact(&mut auth_resp).await.map_err(Error::IoError)?;
            Self::socks5_check_auth(&auth_resp)?;
        }

        // 3. Connect request
        let req = Self::socks5_connect_request(target_host, target_port)?;
        stream.write_all(&req).await.map_err(Error::IoError)?;
        stream.flush().await.map_err(Error::IoError)?;

        // 4. Read response header
        let mut connect_resp = [0u8; 4];
        stream.read_exact(&mut connect_resp).await.map_err(Error::IoError)?;
        if connect_resp[0] != 0x05 || connect_resp[1] != 0x00 {
            return Err(Error::ProxyConnect);
        }
        ...
        match connect_resp[3] {
            0x01 => {
                let mut buf = [0u8; 6];
                stream.read_exact(&mut buf).await.map_err(Error::IoError)?;
            }
            0x03 => {
                let mut len = [0u8; 1];
                stream.read_exact(&mut len).await.map_err(Error::IoError)?;
                let mut buf = vec![0u8; len[0] as usize + 2];
                stream.read_exact(&mut buf).await.map_err(Error::IoError)?;
            }
            0x04 => {
